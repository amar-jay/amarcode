...
import { Bot, Check, CircleDot } from "lucide-react"
...
function SessionEvent({ event, onRespond }: { event: AgentEvent; onRespond: (event: AgentEvent, result: unknown) => void }) {
  ...
  if (event.kind === "activity") return <Tool defaultOpen={false}><ToolHeader type="dynamic-tool" toolName={event.data.label} state="output-available" /><ToolContent><ToolInput input={event.data.payload} /><ToolOutput errorText={undefined} output={event.data.payload} /></ToolContent></Tool>
